...
$periode_result = mysqli_query($conn, "SELECT id, nama_periode FROM periode_penilaian WHERE Status = 'Aktif'");
            ...
            <form id="formPenilaian">
                ...
                <div class="form-group">
                    ...
                    <select name="karyawan_id" id="karyawan_id" class="form-control" required>
                        <option value="">-- Pilih Unit dan Periode Dahulu --</option>
                    </select>
                </div>
...
function loadKaryawan() {
    ...
    const periodeId = document.getElementById('periode_id').value; // <-- Ambil nilai periode
    const karyawanSelect = document.getElementById('karyawan_id');
    
    // Reset dropdown karyawan
    karyawanSelect.innerHTML = '<option value="">-- Pilih Karyawan --</option>';

    // Hanya jalankan fetch jika KEDUA unit dan periode sudah dipilih
    if (unitId && periodeId) { 
        karyawanSelect.innerHTML = '<option value="">Memuat...</option>'; // Loading state
        
        // Kirim kedua parameter ke get_karyawan.php
        fetch(`get_karyawan.php?unit_id=${unitId}&periode_id=${periodeId}`)
            .then(res => {
                if (!res.ok) {
            ...
            .then(data => {
                ...
                if (data.error) {
                    ...
                    karyawanSelect.innerHTML = '<option value="">-- Gagal memuat --</option>';
                } 
                // Cek jika tidak ada karyawan (array kosong)
                else if (data.length === 0) {
                    karyawanSelect.innerHTML = '<option value="">-- Tidak ada karyawan tersedia --</option>';
                } 
                // Jika berhasil dan ada data
                else {
                    karyawanSelect.innerHTML = '<option value="">-- Pilih Karyawan --</option>'; // Reset
                    data.forEach(k => {
            ...
            .catch(err => {
                ...
                karyawanSelect.innerHTML = '<option value="">-- Terjadi error --</option>';
            });
    } else {
         // Jika salah satu belum dipilih, set placeholder
         karyawanSelect.innerHTML = '<option value="">-- Pilih Unit dan Periode Dahulu --</option>';
    }
}

// Tambahkan event listener ke KEDUA dropdown
document.getElementById('unit_id').addEventListener('change', loadKaryawan);
document.getElementById('periode_id').addEventListener('change', loadKaryawan);
</script>

<script>
// Submit Penilaian (Script ini TIDAK BERUBAH)
            ...
            .then(data => {
                Swal.fire({
                    ...
                    if (data.success) {
                        ...
                        // Reset dropdown karyawan kembali ke state awal
                        $('#karyawan_id').html('<option value="">-- Pilih Unit dan Periode Dahulu --</option>');                    
                    }
                });
            })
            .catch(err => {
